...
if [ $# -lt 4 ]; then
    echo "Usage: $0 <CLUSTER_NAME> <NODE_NAME> <NODE_ROLES> <DISCOVERY_SEED_HOSTS>"
    ...
    echo "- NODE_ROLES: node role (e.g., data, master, ingest)"
    echo "- DISCOVERY_SEED_HOSTS: a list of other nodes in the cluster that are master-eligible"
    echo "Example: $0 my-elasticsearch-cluster data-1 data 10.0.0.1,10.0.0.2,10.0.0.3"
    exit 1
fi
...
# Check if CA certificate exists (must be copied from master node)
if [ ! -f ca.crt ] || [ ! -f ca.key ]; then
    echo "ERROR: CA certificate not found!"
    echo "You must copy ca.crt and ca.key from the master node."
    exit 1
fi
